...
//class GRT_STRUCTS_DB_PUBLIC db_ForeignKey : public GrtNamedObject {
    ...
    virtual bool visit(ClassSpecifierAST *ast)
    {
        ...
        //if (className.startsWith(QLatin1String("Grt"))) {
        if (className.startsWith(prefix)) {
            if (className == QLatin1String("Grt")) {
            } else {
...
QStringList inspectGRT_H(const Snapshot &snapshot, const QDir &mwbDir, const QString &dumpersFile, QString& prefixClass)
{
    ...
    mwbDataDir.cd("data");
    //QFileInfo fileGRT_h(mwbDataDir, QLatin1String(GRT_H));
    //QFileInfo fileGRT_h(mwbDataDir, QLatin1String("structs.db.h"));
    ...
    QDir mwbOutDir(mwbDir.absolutePath());
    mwbOutDir.cd("out");
    QFileInfo outFileAST_h(mwbOutDir, QLatin1String(GRT_H));
...
void printUsage()
{
    ...
    const QString defaultpathPhpFrontend
        = QFileInfo(QLatin1String(PATH_MWB_FRONTEND)).canonicalFilePath();
...
int main(int argc, char *argv[])
{
    ...
    QString pathMwbFrontend = QLatin1String(PATH_MWB_FRONTEND);
    ...
    mwbDataDir.cd("data");
    if (!QFileInfo(mwbDataDir, QLatin1String(GRT_H)).exists()) {
